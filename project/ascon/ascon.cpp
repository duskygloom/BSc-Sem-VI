...
ByteArr asconProcessCipher(State state, byte b, ByteArr cipher) {
    ...
    // printf("0x%016lx\n", plain.w[i]);
    // resizeByteArr(&plain, cipher.length);
    // /* pad plain text with 10* */
    // deleteByteArr(&padded);
    // padded = getPaddedByteArr(plain);
    // state[0] ^= padded.w[i];
    // printf("0x%016lx\n", padded.w[i]);
    // deleteByteArr(&padded);
    // /* re-truncate and return */
    // plain.length = cipher.length;
    // resizeByteArr(&plain, cipher.length);
    // return plain;
}
